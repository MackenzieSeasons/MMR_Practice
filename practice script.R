...
analytic.data.practice <- read_csv("mmr_practice_data.csv")
apa.cor.table(analytic.data.practice, table.number = 1)
apa.cor.table(analytic.data.practice, filename="Table1_Practice_APA.doc", table.number=1)

#center variables
analytic.data.practice$x.centered    <- as.numeric(scale(analytic.data.practice$anxiety, center=T, scale=F))
analytic.data.practice$z.centered    <- as.numeric(scale(analytic.data.practice$preparation, center=T, scale=F))
interaction.regression <- lm(exam~x.centered+z.centered+I(x.centered*z.centered), data=analytic.data.practice, na.action = na.exclude)


# I get the table this way -> look here for sr2 for the interaction
apa.reg.table(interaction.regression, table.number = 2)
apa.reg.table(interaction.regression, filename="Table2_Practice_APA.doc", table.number=2)

#I get the values for the text this way
summary(interaction.regression)
# t is 219.7, f is 496 df, p < .0001 t(496) = 219.70


# Surface cross-section line +1 SD  on z (i.e., +1 SD on Preparation) -----------------------------------------
sd.z <- sd(analytic.data.practice$z.centered, na.rm=TRUE)
analytic.data.practice <- analytic.data.practice %>% mutate(z.centered.at.plus.1SD = z.centered - sd.z)

simple.slope.plus.1SD <- lm(exam ~ x.centered + z.centered.at.plus.1SD
                            + I(x.centered*z.centered.at.plus.1SD),
                            data=analytic.data.practice, na.action=na.exclude)

#table this way (look here for CI's at x.centered - anxiety and exam grades)
apa.reg.table(simple.slope.plus.1SD)

#values for text this way - looks for t values and significance
...
analytic.data.practice <- analytic.data.practice %>% mutate(z.centered.at.minus.1SD=z.centered + sd.z)
...
#table this way (look for CI's for x centered, anxiety and exam outcomes when prep is low)
apa.reg.table(simple.slope.minus.1SD)


#values for text this way, gives t values and p values
...
intr.plot(b.0=47.04877,b.x=15.01113,b.z=9.45285,b.xz=22.60574,
...
#set APA part of graph below as well as line adds
